...
#define FONT_SIZE 192
#define CHARACTER_HEIGHT 8
#define CHARACTER_WIDTH 8

const unsigned int toncfontTiles[FONT_SIZE]=
{
...
void unpackFont(u16* destination, u16 palet_id)
{
    for (int i=0; i < FONT_SIZE; i+=2)
    {
        // memory ptr for a character (4bpp or 16 u16 for a tile)
        u16* characterMemory = &(destination[i * 16]);

        // Each character is split in 2 u32 ()
        for (u32 k = 0; k < 2; k++)
        {
            u32 offset = 0;
            u32 character = toncfontTiles[i + k];
            characterMemory = &(destination[(i/2) * 16 + k*8]);
            for (int j=0; j < (CHARACTER_HEIGHT * CHARACTER_WIDTH) / 2; j++)
            {
                u32 block_x = (offset / 4) % 2;
                u32 block_y = offset / 8;
                u32 x = offset % 4;
                u32 value = ((character & (1 << offset)) >> offset) * palet_id;
                characterMemory[block_y * 2 + block_x] += value  << (x*4);
                offset++;
            }
        }
    }
