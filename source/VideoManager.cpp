...
VideoManager::VideoManager()
{
    ...
    for(u32 i = 0; i < 4; i++)
    {
        this->memoryLayout[i] = 0;
    }
}

bool setupPalet4bpp(
    volatile u16* palet_adress, u32 palet_index, const u16* palet, u32 palet_length, u32 offset = 0)
{
...
bool VideoManager::setupObjectPalet4bpp(u32 palet_index, const u16* palet, u32 palet_length)
{ return setupPalet4bpp(object_palet_mem, palet_index, palet, palet_length); }

bool VideoManager::setupBGMPalet4bpp(u32 palet_index, const u16* palet, u32 palet_length)
...
const video_memory_proxy VideoManager::getVideoMemoryTile(MemoryType type, u32 size)
{
    ...
    if(memoryBlock.size != 0) { return memoryBlock; }
...
const video_memory_proxy VideoManager::getVideoMemoryForScreenBlock(u32 screenblock_index)
{
    ...
    {
        LOG_ERROR(
            "Screen block %d is already used for type %d, cannot use it for bgm_map", screenblock_index,
            current_type);
        video_memory_proxy memoryBlock = {.index = 0, .size = 0, .ptr = nullptr};
        return memoryBlock;
    }
