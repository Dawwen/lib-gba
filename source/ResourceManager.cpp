...
{
    char* ptr = (char*) (data->data + ((data->width * data->height * data->atlas_length) / 4)
                         + data->palet_length + data->ani_length);
    // 32bit alignement
    if((u32) ptr % 4) { ptr += 4 - ((u32) ptr % 4); }
    return (animation_resource*) ptr;
}

const animation_resource* ResourceManager::getAnimationResource(u32 id)
{
    ...
    {
        if(ressource->id == id) { return ressource; }
...
u32 ResourceManager::getFrameIndexFromResource(const animation_resource* resource, u32 index)
{
    ...
    const u16* atlas = resource->data + (resource->width * resource->height * resource->atlas_length / 4)
                       + resource->palet_length;
