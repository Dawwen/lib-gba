    ...
    for(auto& freeMemory : freeMemoryList)
    {
        ...
        if(freeMemory.size >= size)
        {
            video_memory_proxy memoryBlock = {
                .sb_index = screenBlockIndex, .index = freeMemory.index, .size = size, .ptr = freeMemory.ptr};

            // If there is  just enough space
            if(freeMemory.size == size) { freeMemoryList.erase(freeMemoryList.begin() + index); }
...
bool MemoryManager::free(video_memory_proxy& memoryBlock)
{
    ...
    for(auto& freeMemory : freeMemoryList)
    {
        if(freeMemory.index < memoryBlock.index) { blockToLeft = &freeMemory; }
...
{
    // The block is full return nullptr
    video_memory_proxy memoryBlock = {.index = 0, .size = 0, .ptr = freeMemoryList[0].ptr};
    return memoryBlock;
}
...
void MemoryManager::reset()
{
    ...
    u16* start_memory_block
        = (u16*) (tile_mem[screenBlockIndex / 8]) + (screenBlockIndex % 8) * sizeof(screen_block);
